...
# Q3 UML 2.5.1 でmoduleという概念がありません。なぜですか。
A3
UML 2.5.1 の仕様において「Module（モジュール）」という単語が独立した主要な要素（メタクラス）として定義されていない理由は、**UMLという言語が「特定のプログラミング言語の実装方式に依存しない（言語中立である）」という設計思想を徹底しているから**です。

具体的には、以下のような「すでに存在する他の概念で完全に代用・表現できる」という背景があります。

---

### 1. 「モジュール」は言語ごとに意味が違いすぎる

プログラミング言語や環境によって、「モジュール」が指す物理的な実体はバラバラです。

* **C言語 / アセンブラ:** `.c` や `.s` などの「ソースファイル単体」
* **Python:** 1つの `.py` ファイル
* **Ruby:** 名前空間やミックスインを提供する「言語機能（`module` 構文）」
* **Java:** `module-info.java` で定義される「複数のパッケージの集合体（Java 9〜）」
* **C++:** `import` される「ソースコードの分離単位（C++20〜）」

もしUMLが安易に「モジュール」という概念を一意に定義してしまうと、特定の言語には適合しても、他の言語では意味が矛盾してしまうことになります。

### 2. UMLには「モジュール」を包括する上位互換の概念がある

UML 2.5.1 では、開発者が「モジュール」と呼びたいものに対して、代わりに以下の3つの概念（要素）を適用するように設計されています。

* **物理的なファイル（`.c` や `.dll` など）としてのモジュール**
👉 **`Artifact`（成果物）** を使います。配置図などで物理的なファイルを表現する要素です。
* **交換可能な機能の塊（カプセル化された部品）としてのモジュール**
👉 **`Component`（コンポーネント）** を使います。内部を隠蔽し、インターフェースを通じてやり取りする抽象度の高い「モジュール」の性質そのものです。
* **名前空間の区切り（フォルダ）としてのモジュール**
👉 **`Package`（パッケージ）** を使います。コードを論理的にカプセル化する役割を持ちます。

つまり、UML 2.5.1 の設計者（OMG）からすると、「わざわざ新しい概念を作らなくても、コンポーネントやアーティファクト、パッケージを使えば、あらゆる言語のモジュールを表現できるよね」という判断なのです。

---

### 補足：それでも「Module」と表現したいときは？

設計書やPlantUMLの図の中で、どうしても「これはUMLの一般的なコンポーネントではなく、プログラミング言語上の『モジュール』である」と明記したい場合は、UMLの拡張機能である **「ステレオタイプ（Stereotype）」** を使用するのが、仕様に準拠した最も正しいアプローチです。

```plantuml
@startuml
' コンポーネントに «module» というステレオタイプ（ラベル）を与える
[z80_io.asm] <<module>> as IO
[main.c] <<module>> as Main
...
このように `«module»` と付記（またはPlantUMLの `rectangle` などで代替）することで、UMLの仕様（中立性）を壊すことなく、C言語やアセンブラの「モジュール図」を表現することができます。
